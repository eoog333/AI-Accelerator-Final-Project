...
- 연구실 PC CUDA 환경에서 최종 MNIST 재학습을 돌릴 수 있도록 `scripts/run_lab_pc.sh` 추가

현재 방향:

- 최종 학습 기준 환경은 연구실 PC이다.
- 지금 저장소를 편집 중인 로컬 환경은 연구실 PC가 아니므로 이 환경에서 측정한 학습 시간/정확도는 최종 실험값으로 쓰지 않는다.
- 연구실 PC 산출물 이름은 `models/mnist_13568_lab.pt`, `results/mnist_metrics_lab.json`, `results/pgm_eval_lab.json`로 통일한다.
...
scripts/run_lab_pc.sh
scripts/check_environment.py
```

로컬 PC, 연구실 PC, CUDA 서버, Jetson Nano 등에서 환경 점검과 실행을 쉽게 하기 위한 스크립트이다.
먼저 `scripts/run_local.sh check`로 Python 패키지, Torch 장치, 모델/영상 파일 존재 여부를 확인한다.
연구실 PC에서는 `scripts/run_lab_pc.sh check`를 먼저 실행하고, 이후 `scripts/run_lab_pc.sh train-mnist`를 사용한다.
...
NUM_WORKERS=4
AMP=1
모델 출력: models/mnist_13568_lab.pt
결과 출력: results/mnist_metrics_lab.json
```
...
따라서 외부 PC나 Jetson Nano로 옮기려면 연구실 PC에서 직접 복사하거나 scp, USB 등을 통해 전달해야 한다.

연구실 PC에서 새로 학습하면 최종 제출에는 아래 산출물을 우선 사용한다.

```text
models/mnist_13568_lab.pt
results/mnist_metrics_lab.json
results/pgm_eval_lab.json
```
...
6. YOLO 공개 가중치가 실제 영상에서 실패할 때만 자체 학습:

```bash
scripts/run_lab_pc.sh yolo-train
```

## 7. Jetson Nano로 옮길 파일
...
4. 연구실 PC에서 MNIST 최종 학습
   - `scripts/run_lab_pc.sh check`로 CUDA와 패키지 상태 확인
   - `scripts/run_lab_pc.sh train-mnist`로 `models/mnist_13568_lab.pt` 생성
   - `results/mnist_metrics_lab.json`의 정확도와 latency를 보고서에 기록

5. Jetson Nano에서 최종 추론이 필요한 경우
   - 연구실 PC에서 만든 `models/mnist_13568_lab.pt`를 Jetson으로 복사
...
아직 완전히 끝난 것은 아니다. 남은 것은 연구실 PC에서 MNIST 최종 재학습을 수행하고, 실제 영상과 YOLO weight를 사용해서 다음 전체 흐름을 실행하는 것이다.
